// A sample of what the piece of shit returns in MEDIA. Only used for reference

[ { id: 14719,
   idMal: 14719,
   format: 'TV',
   nextAiringEpisode: null,
   title:
    { romaji: 'JoJo no Kimyou na Bouken',
      english: 'JoJo\'s Bizarre Adventure',
      native: 'ジョジョの奇妙な冒険',
      userPreferred: 'JoJo no Kimyou na Bouken' },
   description:
    'Beginning its tale in 19th century England, young aristocrat Jonathan Joestar finds himself locked in bitter rivalry with Dio Brando, a low-born boy who Jonathan\'s father took under his wing after the death of Dio\'s father. Discontent with his station in life, Dio\'s fathomless lust to reign over all eventually leads him to seek the supernatural powers of an ancient Aztec stone mask in the Joestar\'s possession—an artifact that will forever change the destiny of Dio and Jonathan for generations to come.<br>\n<br>\nFifty years later, in 1938 New York City, Jonathan\'s grandson Joseph Joestar must take up his grandfather\'s mission and master the abilities necessary to destroy the stone mask and its immensely powerful creators who threaten humanity\'s very existence, the Pillar Men.<br><br>\n(Source: Anime News Network)',
   episodes: 26,
   duration: 24,
   genres: [ 'Action', 'Adventure', 'Supernatural' ],
   averageScore: 81,
   status: 'FINISHED',
   coverImage:
    { medium:
       'https://s4.anilist.co/file/anilistcdn/media/anime/cover/small/n14719-EVJmyKNzcjet.jpg',
      large:
       'https://s4.anilist.co/file/anilistcdn/media/anime/cover/medium/n14719-EVJmyKNzcjet.jpg',
      color: '#e4a15d' },
   startDate: { year: 2012, month: 10, day: 6 },
   endDate: { year: 2013, month: 4, day: 6 },
   relations: { edges: [Array] },
   characters: { nodes: [Array] } }]